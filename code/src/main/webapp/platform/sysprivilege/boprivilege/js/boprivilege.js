$(function(){
			$('#privilegeGrid').datagrid({
				...
				nowrap: true,
				striped: true,
				url:'platform/sysprivilege/boprivilege/list.action',
		...
		function reloadgrid(){
	        ...
	        var privilegeOpClass = $("#privilegeOpClass").val();
	        var privilegeOpMethod = $("#privilegeOpMethod").val();
			queryParams.privilegeName=privilegeName;
			queryParams.privilegeOpClass=privilegeOpClass;
			queryParams.privilegeOpMethod=privilegeOpMethod;
...
function editInfoType() {
	...
	if(num<1){
		...
		$("#id").val(rows[0].id);
		$("#privilegeForm").attr('action','platform/sysprivilege/boprivilege/input.action');
...
function deleteinfoType() {
		...
		if(num < 1){
			...
			$.messager.confirm('确认提示','你确定要删除该数据吗?',function(r){ 
			    if(r){
						$.ajax({
						    url:'platform/sysprivilege/boprivilege/delete.action',
						    type:'post',
						    data:{id:ids},
						    error:function(){
