$(function(){
			$('#privilegeGrid').datagrid({
				...
				nowrap: true,
				striped: true,
				url:'platform/sysprivilege/urlprivilege/list.action',
...
function editInfoType() {
	...
	if(num<1){
		...
		$("#id").val(rows[0].id);
		$("#privilegeForm").attr('action','platform/sysprivilege/urlprivilege/input.action');
...
function deleteinfoType() {
		...
		if(num < 1){
			...
			$.messager.confirm('确认提示','你确定要删除该数据吗?',function(r){ 
			    if(r){
						$.ajax({
						    url:'platform/sysprivilege/urlprivilege/delete.action',
						    type:'post',
						    data:{id:ids},
						    error:function(){
