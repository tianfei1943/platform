$(function(){
			$('#resourceGrid').datagrid({
				...
				nowrap: true,
				striped: true,
				url:'platform/sysresource/menu/list.action',
		...
		{title:'资源路径',field:'resourceUrl',width: 300, formatter : function(value) {
			if (value == null) {
				return "";
			}
			return value;
		...
		{title:'资源描述',field:'desciption', width: 200, formatter : function(value) {
			...
			return value;
				...
				pagination:true,
				queryParams:{"resourceName":""},//查询和排序
				rownumbers:true
				//pageSize:10,
				//pageNumber:1
				//singleSelect:true
...
function editInfoType() {
	...
	if(num<1){
		...
		$("#id").val(rows[0].id);
		$("#menusForm").attr('action','platform/sysresource/menu/input.action');
		$("#menusForm").submit();
	}
}
...
function deleteinfoType() {
	...
	if (btn) {
		$.ajax({
			url:'platform/sysresource/menu/delete.action',
			type:'post',
			data:{id:ids},
			error:function(){
